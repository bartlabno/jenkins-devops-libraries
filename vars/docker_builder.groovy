...
def call(Map buildParams) {
    node ( label: 'linux' ) {
        ...
        stage('publish') {
            sh script: "docker tag ${projectName} \$(aws sts get-caller-identity | jq -r .Account).dkr.ecr.eu-west-2.amazonaws.com/${projectName}:\$(echo $BRANCH_NAME)-\$(echo $BUILD_NUMBER)", label: "tag image"
