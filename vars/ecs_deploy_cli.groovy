        ...
        stage("gathering fatcs") {
            ...
            defaults.environments.each { envs ->
                ...
                if (!pipe_vars.vpc) { 
                    ...
                    sh "export DEFAULT_SUBNETS=\$(aws ec2 describe-subnets --region ${defaults.awsRegion} --filters Name=vpc-id,Values=${DEFAULT_VPC} --output text --query Subnets[].SubnetId)"
